...
async def check_text_result(result: models.QueryResult, payload: dict, task_config: models.OrchestratorServerConfig) -> Union[float, None]:
    ...
    for counter, index in enumerate(indicies_to_check):
        ...
        logger.info(f"counter: {counter}; index: {index}; distance: {distance}; finish_reason_vali: {finish_reason_vali}; finish_reason: {finish_reason}")
        ...
        if counter == len(indicies_to_check) - 1:
            logger.info(f"Validator's LLM checking server's last token indicates finish reason {finish_reason_vali} but miner implies {finish_reason}")
            try:
                assert finish_reason == finish_reason_vali
            except AssertionError as e:
                logger.error(f"Validator's LLM checking server's last token indicates finish reason {finish_reason_vali} but miner implies {finish_reason} ; Error: {e}")
                return 0
        checks += 1
