...
def test_runpod_orch(instance_id, orch_port, test_json_file):
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python ci-tests/test_runpod_orch.py <function_name> [args...]")
        sys.exit(1)

    function_name = sys.argv[1]

    if function_name == "test_runpod_orch":
        if len(sys.argv) != 5:
            print("Usage: python ci-tests/test_runpod_orch.py test_runpod_orch <INSTANCE_ID> <ORCH_PORT> <TEST_JSON_FILE>")
            sys.exit(1)
        instance_id = sys.argv[2]
        orch_port = sys.argv[3]
        test_json_file = sys.argv[4]
        test_runpod_orch(instance_id, orch_port, test_json_file)
    elif function_name == "test_runpod_orch_image":
        if len(sys.argv) != 5:
            print("Usage: python ci-tests/test_runpod_orch.py test_runpod_orch_image <INSTANCE_ID> <ORCH_PORT> <TEST_JSON_FILE>")
            sys.exit(1)
        instance_id = sys.argv[2]
        orch_port = sys.argv[3]
        test_json_file = sys.argv[4]
        ...
        if len(sys.argv) != 5:
            print("Usage: python ci-tests/test_runpod_orch.py load_model <INSTANCE_ID> <LLM_PORT> <TEST_JSON_FILE>")
            sys.exit(1)
        instance_id = sys.argv[2]
        llm_port = sys.argv[3]
        test_json_file = sys.argv[4]
        ...
        if len(sys.argv) != 5:
            print("Usage: python ci-tests/test_runpod_orch.py test_llm <INSTANCE_ID> <LLM_PORT> <TEST_JSON_FILE>")
            sys.exit(1)
        instance_id = sys.argv[2]
        llm_port = sys.argv[3]
        test_json_file = sys.argv[4]
